 ...
 * perspective of the
 * plant. This means U is an input and Y is an output (because you give the
 * plant U (powers) and it
 * gives you back a Y (sensor values)). This is the opposite of what they mean
 * from the perspective
 * of the controller (U is an output because that's what goes to the motors and
 * Y is an input
 ...
 */
public class AngleWrapLinearSystemLoop<States extends Num, Inputs extends Num, Outputs extends Num> {
     ...
     */
    public AngleWrapLinearSystemLoop(
            ...
            ArrayList<Integer> m_angleIndexes) {
    ...
    public void predict(double dtSeconds) {
        var error = m_nextR.minus(getObserver().getXhat());
        var k = m_controller.getK();
        for (Integer idx : m_AngleIndexes) {
            error.set(idx, 0, MathUtil.angleModulus(error.get(idx, 0)));
        }
        var u_back = k.times(error);
